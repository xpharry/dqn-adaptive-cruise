...
def generate_circle_track(radius, nedges):
    poses = []
    ...
    for i in range(nedges):
        ...
        pose.position.x = 2.0*i*radius/nedges - radius
        pose.position.y = 0.0
    ...
    for i in range(nedges/2):
        ...
        pose.position.x = radius * cos(phi) + radius
...
def main():
    poses = generate_circle_track(RADIUS, NEDGES)
    write_to_csv(poses)
