...
{ // constructor
    ...
    */
    ...
    des_state_ = current_gps_;  // use the current odom state
...
void SteeringController::initializeSubscribers() {
    ...
    gps_subscriber_ = nh_.subscribe("/gazebo_mobot_pose", 1, &SteeringController::gpsCallback, this); //subscribe to gps messages
...
void SteeringController::gpsCallback(const nav_msgs::Odometry& gps_rcvd) {
    // copy some of the components of the received message into member vars
    // we care about speed and spin, as well as position estimates x,y and heading
    current_odom_ = gps_rcvd; // save the entire message
    // but also pick apart pieces, for ease of use
    odom_pose_ = gps_rcvd.pose.pose;
    odom_vel_ = gps_rcvd.twist.twist.linear.x;
    odom_omega_ = gps_rcvd.twist.twist.angular.z;
    odom_x_ = gps_rcvd.pose.pose.position.x;
    odom_y_ = gps_rcvd.pose.pose.position.y;
    odom_quat_ = gps_rcvd.pose.pose.orientation;
    //odom publishes orientation as a quaternion.  Convert this to a simple heading
    odom_phi_ = convertPlanarQuat2Phi(odom_quat_); // cheap conversion from quaternion to heading for planar motion
    // let's put odom x,y in an Eigen-style 2x1 vector; convenient for linear algebra operations
    //odom_xy_vec_(0) = odom_x_;
